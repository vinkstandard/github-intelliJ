...
//        in some languages this "list" is in fact a string (see the Sample Tests).
    ...
    static int sommaMigliore = -1;
    public static Integer chooseBestSum(int t, int k, List<Integer> ls) {

        // t == distanza massima che vogliono percorrere
        // k == numero di paesi
        // ls lista
    ...
    private static void explore(List<Integer> ls, int k, int inizio, int sommaAttuale, int t) {
        if (k == 0) {
            if (sommaAttuale <= t && sommaAttuale > sommaMigliore) {
                sommaMigliore = sommaAttuale;
            }
            return;
        }
        for (int i = inizio; i < ls.size(); i++) {
            explore(ls, k - 1, i + 1, sommaAttuale + ls.get(i), t);
        }
    }
}
